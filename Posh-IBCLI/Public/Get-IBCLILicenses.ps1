function Get-IBCLILicenses
{
    ...
    Write-Verbose "Fetching 'show license csv' output from $ComputerName"
    <#
    ...
    try {
        ...
        $hwid = Get-IBCLIHardwareID $ComputerName $Credential
        Write-Verbose $hwid
        ...
        $ip = (Get-IBCLIStatus $ComputerName $Credential).IPAddress

        # get the command output and parse the csv
        $output = Invoke-IBCLICommand 'show license csv' $stream
